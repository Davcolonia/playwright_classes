...
class CustomTTAReporter implements Reporter {
    ...
    constructor(options: ReporterOptions = {}) {
        this.options = {
            screenshot: { enabled: true, outputDir: 'screenshots', embedBase64: false, ...options.screenshot },
            trace: { enabled: true, outputDir: 'traces', openViewer: true, ...options.trace },
        };
    }
    ...
    onBegin(config: FullConfig, suite: Suite): void {
        ...
        this.logConfigGuidance(config);
    }

    private logConfigGuidance(config: FullConfig): void {
        const hasScreenshot = config.projects.some(p => p.use?.screenshot);
        const hasTrace = config.projects.some(p => p.use?.trace);

        if (!hasScreenshot && this.options.screenshot?.enabled) {
            console.log('💡 TIP: Enable screenshots by adding `screenshot: "on"` or `screenshot: "only-on-failure"` to `use` in playwright.config.ts');
        }
        if (!hasTrace && this.options.trace?.enabled) {
            console.log('💡 TIP: Enable traces by adding `trace: "on"` or `trace: "retain-on-failure"` to `use` in playwright.config.ts');
        }
        if (this.options.trace?.openViewer) {
            console.log('🔍 Traces can be viewed at: https://trace.playwright.dev');
        }
    }
    ...
    onTestEnd(test: TestCase, result: TestResult): void {
        ...
        const allLogs = this.associateLogsWithSteps(test, result, currentTestSteps);
        ...
        const screenshotOutputDir = this.options.screenshot?.outputDir || 'screenshots';
        const traceOutputDir = this.options.trace?.outputDir || 'traces';

        for (const attachment of result.attachments) {
            // ── Screenshots: handle png, jpeg, webp ──
            const isImage = /^image\/(png|jpeg|webp)$/.test(attachment.contentType);
            if (isImage) {
                const ext = attachment.contentType.split('/')[1] === 'jpeg' ? 'jpg' : attachment.contentType.split('/')[1];
                const screenshotName = `screenshot_${this.testCounter}_${screenshots.length + 1}.${ext}`;
                const destPath = path.join('tta-report', screenshotOutputDir, screenshotName);
                ...
                try {
                    ...
                    screenshots.push({ name: attachment.name || `Screenshot ${screenshots.length + 1}`, path: `${screenshotOutputDir}/${screenshotName}` });
                    if (attachment.name) {
                        stepScreenshots.set(attachment.name, `${screenshotOutputDir}/${screenshotName}`);
                    }
                } catch {
            ...
            // ── Trace: detect by name OR zip content-type ──
            const isTraceByName = attachment.name === 'trace' || /^trace/.test(attachment.name || '');
            const isTraceByType = /^application\/(zip|x-zip-compressed|octet-stream)$/.test(attachment.contentType);
            const isZipFile = /\.zip$/i.test(attachment.name || '');

            if ((isTraceByName || (isTraceByType && isZipFile)) && (attachment.path || attachment.body)) {
                const traceName = `trace_${this.testCounter}.zip`;
                const destPath = path.join('tta-report', traceOutputDir, traceName);
                const destDir = path.dirname(destPath);
                if (!fs.existsSync(destDir)) {
                    fs.mkdirSync(destDir, { recursive: true });
                }
                try {
                    ...
                    tracePath = `${traceOutputDir}/${traceName}`;
                } catch {
                    console.warn(`Failed to copy trace: ${attachment.path || attachment.name}`);
                }
            }
        }
    ...
    private associateLogsWithSteps(_test: TestCase, result: TestResult, testSteps: StepData[]): string[] {
...
<body>
        ...
        for (const test of this.testResults) {
            ...
            html += `
                <tr class="test-row ${statusClass}" data-test-id="${test.id}" data-tags="${tagsData}">
                    ...
                    <td class="col-trace">
                        ${test.trace ? `<a href="https://trace.playwright.dev?trace=${encodeURIComponent(test.trace)}" target="_blank" class="trace-link-cell">📁 View</a>` : 'N/A'}
                    </td>
                </tr>
    ...
    private generateTestDetailPanel(test: TestData): string {
        ...
        if (test.logs.length > 0) {
            ...
            <div class="detail-section console-section">
                <div class="section-header" onclick="toggleSection(this)">
                    <span class="section-arrow">▼</span> Full Console Output (${test.logs.length} lines)
                </div>
                <div class="section-content">
                    <div class="full-console-content">`;
            ...
            <div class="detail-section traces-section">
                ...
                <div class="section-content">
                    <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
                        <a href="${test.trace}" download class="trace-download">📁 Download Trace</a>
                        <a href="https://trace.playwright.dev?trace=${encodeURIComponent(test.trace)}" target="_blank" class="trace-viewer-link">🔍 Open in Trace Viewer</a>
                    </div>
                    <p style="margin-top: 8px; font-size: 12px; color: #64748b;">Drag & drop the downloaded trace file onto <a href="https://trace.playwright.dev" target="_blank" style="color: #059669;">trace.playwright.dev</a></p>
                </div>
            </div>`;
        }
    ...
    public getTraceViewerUrl(tracePath: string): string {
    ...
    private getStyles(): string {
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .trace-viewer-link {
            ...
            gap: 8px;
            padding: 12px 20px;
            background: linear-gradient(135deg, #ede9fe 0%, #d4c5f9 100%);
            color: #7c3aed;
            text-decoration: none;
            border-radius: var(--radius-sm);
            font-size: 13px;
